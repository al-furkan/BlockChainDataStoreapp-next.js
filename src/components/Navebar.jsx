    ...
    <nav className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            {role === "admin" && (
              <Link href="/dashboard/admin" className="hover:text-yellow-300 transition">
                Admin Dashboard
              </Link>
            )}

            {role === "user" && (
              <Link href="/dashboard/user" className="hover:text-yellow-300 transition">
                User Dashboard
              </Link>
            )}
      ...
      {isOpen && (
        <div className="md:hidden bg-indigo-800 space-y-1 pb-3">
          ...
          {!role && (
            <>
              <Link href="/register" className="block px-4 py-2 hover:bg-blue-700">
                Register
              </Link>
              <Link href="/login" className="block px-4 py-2 hover:bg-blue-700">
                Login
              </Link>
            </>
          )}
          ...
          {role === "admin" && (
            <Link href="/dashboard/admin" className="block px-4 py-2 hover:bg-blue-700">
              Admin Dashboard
            </Link>
          )}

          {role === "user" && (
            <Link href="/dashboard/user" className="block px-4 py-2 hover:bg-blue-700">
              User Dashboard
            </Link>
          )}
