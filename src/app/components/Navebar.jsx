    ...
    <nav className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            {user && (
              <button onClick={handleLogout} className="ml-4 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-full text-sm font-medium transition">
                Logout
              </button>
            )}
            ...
            <Link href="/help" className="ml-4 bg-yellow-400 hover:bg-yellow-500 text-gray-900 px-4 py-2 rounded-full text-sm font-medium transition">Help</Link>
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-indigo-800 space-y-1 pb-3">
          ...
          {user?.role === "admin" && <Link href="/admin/dashboard" className="block px-4 py-2 hover:bg-blue-700">Admin Dashboard</Link>}
          {user?.role === "user" && <Link href="/user/dashboard" className="block px-4 py-2 hover:bg-blue-700">User Dashboard</Link>}
          {user && <button onClick={handleLogout} className="block w-full text-left px-4 py-2 hover:bg-red-600">Logout</button>}
          ...
          <div className="px-4 mt-3">
            <Link href="/help" className="block w-full text-center bg-yellow-400 hover:bg-yellow-500 text-gray-900 px-4 py-2 rounded-full text-sm font-medium transition">Help</Link>
          </div>
        </div>
      )}
    </nav>
